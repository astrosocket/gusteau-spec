...
It would be a good candidate for moving to the `FromParent/gas` subgroup.

Note that entire subgroups can be retained in this manner and we recommend retaining any group hierarchy, just under the `FromParent/${ParticleType}/` subgroup instead of the top-level group. 

Likewise, relevant tags can be moved to the `FromParent/${ParticleType}` group instead of remaining at the top-level.
